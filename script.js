    ...
    removeButton[i].addEventListener('click', function(){
        ...
        if(count[i].innerText<=0){
            ...
            if(i==0){
                ...
                decrease.innerText = `${count[i].innerText} X 100`
            }
            else if(i==1){
                let decrease = cartContent.querySelector(`.second p`);
                decrease.innerText = `${count[i].innerText} X 200`
            }
            else{
                let decrease = cartContent.querySelector(`.third p`);
                decrease.innerText = `${count[i].innerText} X 300`
            }
        }
        if(amount>0){
    ...
    addButton[i].addEventListener('click', function(){
        ...
        if(count[i].innerText==1){
            ...
            if(i==0){
                ...
                increase.innerText = `${count[i].innerText} X 100`
            }
            else if(i==1){
                let increase = cartContent.querySelector(`.second p`);
                increase.innerText = `${count[i].innerText} X 200`
            }
            else{
                let increase = cartContent.querySelector(`.third p`);
                increase.innerText = `${count[i].innerText} X 300`
            }
        }
        amount += parseInt(price[i].innerText);
        totalAmount.innerText =  `₹ ${amount}`
